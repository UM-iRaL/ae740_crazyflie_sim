...
class Commander():
    ...
    def send_force_torque_setpoint(self, thrust_si, torque_x, torque_y, torque_z):
        """
        Send direct force and torque commands to the Crazyflie.
        
        Args:
            thrust_si (float): Thrust in Newtons
            torque_x (float): Torque around X-axis in Newton-meters
            torque_y (float): Torque around Y-axis in Newton-meters  
            torque_z (float): Torque around Z-axis in Newton-meters
        """
        pk = CRTPPacket()
        pk.port = CRTPPort.COMMANDER_GENERIC
        pk.channel = SET_SETPOINT_CHANNEL
        
        # Pack the data: type (1 byte) + 4 floats (4 bytes each)
        pk.data = struct.pack('<Bffff', TYPE_FORCE_TORQUE, thrust_si, torque_x, torque_y, torque_z)
