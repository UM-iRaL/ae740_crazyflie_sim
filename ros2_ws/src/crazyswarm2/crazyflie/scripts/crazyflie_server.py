...
from crazyflie_interfaces.msg import NamedPoseArray
...
class CrazyflieServer(Node):
    def __init__(self):
        ...
        for uri in self.cf_dict:
            ...
            self.create_subscription(
                ForceTorqueCmd, name +
                "/force_torque_cmd", partial(self._cmd_force_torque, uri=uri), 10
            )
            self.create_subscription(
                Hover, name +
    ...
    def _init_default_logblocks(self, prefix, link_uri, list_logvar, global_logging_enabled, topic_type):
        ...
        try:
            ...
            # self.get_logger().info("error 1 " + str(cf_name) + " " + str(topic_type))
            pass
        try:
            ...
            self.get_logger().info("error 2 " + str(cf_name) + " " + str(topic_type))
            pass
        try:
            ...
            # self.get_logger().info("error 3 " + str(cf_name) + " " + str(topic_type))
            pass
        ...
        if logging_enabled and global_logging_enabled:
            ...
            self.get_logger().info("Empty publisher for " + str(topic_type) + " " + str(cf_name))
    ...
    def _log_velocity_data_callback(self, timestamp, data, logconf, uri):
        """
        Once velocity data is retrieved from the Crazyflie, 
            send out the ROS 2 topic for Twist
        """
        ...
        # if data.get('kalman.statePX') is None:
        #     self.get_logger().info(f"Velocity data is None for {uri}")
        
        # if isnan(data.get('kalman.statePX')):
        #     self.get_logger().info(f"Velocity data is NaN for {uri}")

        # self.get_logger().info(f"Velocity data: {data.get('kalman.statePX')} and uri is {uri}")
        ...
        self.swarm._cfs[uri].logging["twist_publisher"].publish(msg)
    ...
    def _cmd_force_torque(self, msg, uri=""):
        try:
            self.swarm._cfs[uri].cf.commander.send_force_torque_setpoint(
                        msg.thrust_si,
                        msg.torque_x, 
                        msg.torque_y,
                        msg.torque_z
