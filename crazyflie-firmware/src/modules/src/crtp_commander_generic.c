...
/* forceTorqueDecoder
 * Direct force/torque control bypassing all intermediate control loops
 */
struct forceTorquePacket_s {
  float thrustSi;   // Thrust in Newtons
  float torqueX;    // Torque around X-axis (Nm)
  float torqueY;    // Torque around Y-axis (Nm) 
  float torqueZ;    // Torque around Z-axis (Nm)
} __attribute__((packed));

static void forceTorqueDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
{
  const struct forceTorquePacket_s *values = data;
  
  ASSERT(datalen == sizeof(struct forceTorquePacket_s));
  
  // Disable all position/attitude control modes
  setpoint->mode.x = modeDisable;
  setpoint->mode.y = modeDisable; 
  setpoint->mode.z = modeDisable;
  setpoint->mode.roll = modeDisable;
  ...
  // Store force/torque values (repurpose existing fields)
  setpoint->thrust = values->thrustSi;
  setpoint->attitudeRate.roll = values->torqueX;
  setpoint->attitudeRate.pitch = values->torqueY;
  setpoint->attitudeRate.yaw = values->torqueZ;
} 
...
const static packetDecoder_t packetDecoders[] = {
  ...
  [forceTorqueType]   = forceTorqueDecoder,
};
